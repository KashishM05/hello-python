pipeline {
    ...
    environment {
        ...
        SONAR_HOST_URL = 'http://34.41.178.220:9000' //Jenkin VM IP
    }
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main',
                    url: 'https://github.com/aditidraut46/hello-python.git'  //github username
            }
        }
            ...
            steps {
                sshagent(credentials: ['gce-ssh']) {
                    sh '''
                        ...
                        ssh -o StrictHostKeyChecking=no aditidraut46@34.16.36.23 "mkdir -p /home/aditidraut46/app"    //appvm (to check IP address whoami)
